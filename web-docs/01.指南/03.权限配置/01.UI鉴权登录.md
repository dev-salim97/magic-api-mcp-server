...
对于需要单独访问的，需要将应用的权限校验对路径/**放行，交给magic-api处理，其它无需配置
...
<iframe src="http://xxxx.xx/index.html"></iframe>
<script>
    // 当跨域时，iframe内是访问不到此处配置的。请改为后台配置，
    // 对于需要在header动态添加值时，可通过 http://xxxx.xx/index.html?headerName=token&headerValue=value 的方式进行添加
